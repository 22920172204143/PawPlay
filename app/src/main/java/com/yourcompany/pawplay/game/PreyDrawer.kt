...
object PreyDrawer {
    ...
    fun drawPrey(
        ...
        canvas.rotate(heading + bodyWobble)
    ...
    private fun cockroach(c: Canvas, s: Float, ph: Float) {
        ...
        for (i in -1..1) {
            val ly = i * bh * 0.32f; val d = if (i % 2 == 0) 1 else -1
            c.drawLine(-bw, ly, -bw - s * 0.24f, ly + lw * d, stroke)
            c.drawLine(bw, ly, bw + s * 0.24f, ly - lw * d, stroke)
        }
    ...
    private fun bee(c: Canvas, s: Float, ph: Float) {
        ...
        c.save(); c.rotate(wa, -s * 0.08f, -s * 0.2f)
        c.drawOval(-s * 0.28f, -s * 0.42f, 0f, -s * 0.04f, fill); c.restore()
        c.save(); c.rotate(-wa, s * 0.08f, -s * 0.2f)
        c.drawOval(0f, -s * 0.42f, s * 0.28f, -s * 0.04f, fill); c.restore()
    ...
    private fun bird(c: Canvas, s: Float, ph: Float) {
        ...
        val wa = sin(ph * 7f).toFloat() * 28f

        // Left wing — organic feathered shape
        c.save(); c.rotate(wa, -s * 0.04f, -s * 0.02f)
    ...
    private fun yarnBall(c: Canvas, s: Float, ph: Float) {
        ...
        drawGlow(c, 0f, 0f, r * 1.8f, Color.rgb(255, 140, 40), ph)

        c.save(); c.rotate(ph * 38f)
        fill.shader = RadialGradient(-r * 0.3f, -r * 0.3f, r * 1.6f,
            Color.rgb(255, 150, 48), Color.rgb(200, 78, 0), Shader.TileMode.CLAMP)
        c.drawCircle(0f, 0f, r, fill); fill.shader = null

        stroke.color = Color.rgb(240, 120, 20); stroke.strokeWidth = 2.2f
        for (i in 0..6) {
        ...
        fill.shader = RadialGradient(-r * 0.25f, -r * 0.25f, r * 0.5f,
            Color.argb(100, 255, 255, 255), Color.argb(0, 255, 255, 255), Shader.TileMode.CLAMP)
        c.drawCircle(-r * 0.25f, -r * 0.25f, r * 0.45f, fill); fill.shader = null
        c.restore()

        stroke.color = Color.rgb(255, 140, 32); stroke.strokeWidth = 2.5f
